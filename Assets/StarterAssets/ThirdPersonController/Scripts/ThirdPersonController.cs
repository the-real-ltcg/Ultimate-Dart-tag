...
namespace StarterAssets
{
    ...
    public class ThirdPersonController : NetworkBehaviour
    {
        ...
        public float SprintSpeed = 600.335f;

        [Tooltip("How fast the character turns to face movement direction")]
        [Range(0.0f, 0.3f)]
        public float RotationSmoothTime = 0.27f;
        ...
        private void OnCollisionEnter(Collision collision)
        {
            ...
            // If we run into another player, check to see if they're tagged. If they are, untag them.
            if (collision.gameObject.CompareTag("Player"))
            {
                var player = collision.gameObject.GetComponent<ThirdPersonController>();
                if (player != null)
                {
                    player.TagPlayerServerRpc(false);
                }
            }
        }

        [ServerRpc]
        public void TagPlayerServerRpc(bool tagged)
        {
            TagPlayerClientRpc(tagged);
        }   

        [ClientRpc]
        private void TagPlayerClientRpc(bool tagged)
        {
            _isTagged = tagged;
        }   
        ...
        private void Move()
        {
            ...
            // But if we're tagged, we can only turn around in place.
            float targetSpeed = _isTagged ? 0.0f : _input.sprint ? SprintSpeed : MoveSpeed;
